...
export default function MyTicketsPage() {
  ...
  const upcomingTickets = registrations?.filter(
    (reg: Registration) =>
      reg.event && reg.event.startTime >= now && reg.status === "confirmed"
  );
  const pastTickets = registrations?.filter(
    (reg: Registration) =>
      reg.event && (reg.event.startTime < now || reg.status === "cancelled")
  );
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {upcomingTickets?.length > 0 && (
          <div className="mb-12">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {upcomingTickets.map((registration: Registration) => (
                <EventCard
        ...
        {pastTickets?.length > 0 && (
          <div>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {pastTickets.map((registration: Registration) => (
                <EventCard
                  key={registration._id}
                  event={registration.event}
                  action={null}
            ...
            <div className="space-y-4">
              ...
              <div className="bg-muted p-4 rounded-lg space-y-2 text-sm">
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    {format(selectedTicket.event.startTime, "PPP, h:mm a")}
                  </span>
                </div>
                <div className="flex items-center gap-2">
