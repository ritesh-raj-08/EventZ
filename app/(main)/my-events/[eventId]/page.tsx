...
import { useConvexQuery as useConvexQueryHook, useConvexMutation } from "@/hooks/use-convex-query";
...
export default function EventDashboardPage() {
  ...
  const slug = params.eventId; // This is actually the slug from the URL

  // First, fetch the event by slug to get the actual Convex ID
  const { data: foundEvent, loading: loadingEvent } = useConvexQuery(
    api.events.getEventBySlug,
    { slug }
  );

  // Use the actual eventId from the found event
  const eventId = foundEvent?._id;

  // If no eventId yet, stay in loading
  if (!eventId) {
    if (loadingEvent) {
      return (
  ...
  // Once we have eventId, render the dashboard with real data
  return <EventDashboardContent eventId={eventId} />;
}

function EventDashboardContent({ eventId }: { eventId: Id<"events"> }) {
  const router = useRouter();
  ...
  useEffect(() => {
    if (dashboardData && currentUser) {
      const isOwner = dashboardData.event.organizerId === currentUser._id;
      if (isOwner) {
        setIsAuthorized(true);
        setAuthError(null);
      } else {
        setIsAuthorized(false);
        setAuthError("You don't have permission to manage this event.");
        // Redirect after a short delay
        setTimeout(() => router.push("/my-events"), 2000);
      }
    }
  }, [dashboardData, currentUser, router]);
  ...
  useEffect(() => {
    if (dashboardData !== undefined) {
      ...
      console.log("⏳ Dashboard data loading or null...");
    }
  }, [dashboardData]);
  ...
  useEffect(() => {
    if (registrations !== undefined) {
      ...
      const checkedInCount = registrations.filter((r: Registration) => r.checkedIn).length;
      console.log(`✅ Checked in: ${checkedInCount}/${registrations.length}`);
      registrations.forEach((reg: Registration, idx: number) => {
        if (reg.checkedIn) {
          console.log(`  [${idx}] ${reg.attendeeName} - CHECKED IN at ${new Date(reg.checkedInAt || 0).toLocaleTimeString()}`);
        }
      });
    } else {
      console.log("⏳ Registrations loading or null...");
    }
  }, [registrations]);
  ...
  if (dashboardData === undefined || registrations === undefined || !currentUser) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <Loader2 className="w-8 h-8 animate-spin text-purple-500" />
      </div>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="max-w-md text-center space-y-4">
          ...
          <p className="text-sm text-muted-foreground">Redirecting to My Events...</p>
        </div>
      </div>
    );
  }

  const { event, stats } = dashboardData as DashboardData;

  // Filter registrations based on active tab and search
  const filteredRegistrations = (Array.isArray(registrations) ? registrations : []).filter((reg: Registration) => {
    const matchesSearch =
  ...
  return (
    <div className="min-h-screen pb-20 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Quick Actions - Show QR Scanner for current + upcoming events (not past) and if authorized */}
        {!stats.isEventPast && isAuthorized && (
          <Button
            size="lg"
            // variant="outline"
